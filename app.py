...
class DeptSales(Resource):
    def get(self):
        ...
        return Response(
            json_util.dumps(response_data, ensure_ascii=False),
            mimetype="application/json; charset=utf-8",
        )
...
class SaleCompare(Resource):
    def get(self):
        ...
        return Response(
            json_util.dumps(transformed, ensure_ascii=False),
            mimetype="application/json; charset=utf-8",
        )
...
class SalesAmtBest(Resource):
    def get(self):
        SalesAmtBest_col = mongo_db["salesAmtBest"]
        results = list(SalesAmtBest_col.find({}, {"_id": 0}))
        if not results:
            abort(404, message="No SalesAmtBest data found")
...
class SalesCntBest(Resource):
    def get(self):
        SalesCntBest_col = mongo_db["salesCntBest"]
        results = list(SalesCntBest_col.find({}, {"_id": 0}))
        if not results:
            abort(404, message="No SalesCntBest data found")
...
api.add_resource(SalesAmtBest, "/abest")
api.add_resource(SalesCntBest, "/cbest")
